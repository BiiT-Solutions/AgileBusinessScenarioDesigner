...
public class DroolsParser {
	...
	private static String assignationFunctionAction(ExpressionChain actions)
			...
			NullCustomVariableException, NullExpressionValueException, DroolsRuleCreationException {
		String ruleCore = "";
		...
		if (((ExpressionChain) chainList.get(0)).getExpressions().get(0) instanceof ExpressionValueCustomVariable) {
			...
			default:
				throw new DroolsRuleCreationException(
						"Error parsing an Action. Function '" + function.getValue() + "' not found.", actions);
			}
			...
			if (rule != null) {
				...
				if (parsedRule != null) {
					...
					if (rule instanceof DroolsRuleGroupEndRule) {
						parsedText.append(
								RuleGenerationUtils.getGroupEndRuleExtraCondition((DroolsRuleGroupEndRule) rule));
					}
			...
			if (checkExpressionValueTypes(getExpressionValueTreeObjectAnswerFormat(leftExpressionValueTreeObject),
				...
				for (ExpressionValue<?> value : operatorValues) {
					if (value instanceof ExpressionValueTreeObjectReference) {
						functionValues.add(getDroolsVariableValueFromExpressionValueTreeObject(
								(ExpressionValueTreeObjectReference) value));
						...
						functionValues
								.add("(new Date(" + ((ExpressionValueTimestamp) value).getValue().getTime() + "l))");
					} else {
				...
				if (!functionValues.isEmpty()) {
					if (leftExpressionValueTreeObject instanceof ExpressionValueCustomVariable) {
						...
						case FORM:
							putTreeObjectName(leftVariable.getReference(),
							...
							default:
								// Never gets here
								throw new DroolsRuleCreationException(
										"No valid function '" + functionParsed.toString() + "'found.", conditions);
							}
						...
						case QUESTION:
							...
							droolsConditions += "\t$" + leftVariable.getReference().getUniqueNameReadable()
									...
									+ RuleGenerationUtils.returnSimpleTreeObjectNameFunction(
											leftVariable.getReference())
					...
					throw new DroolsRuleCreationException(
							"No valid values found inside the function '" + functionParsed.toString() + "'",
							conditions);
				}
			}
		} else {
	...
	/**
	 * Parse conditions like => Score (logic operator (==, <=,
	 * <, >=, >)) value. <br> Create drools rule like => Category(
	 * isVariableDefined('cScore'), getVariablevalue('cScore') == value )
	 * 
	 * @param conditions
	 * @return LHS of the rule
	 * @throws TreeObjectParentNotValidException
	...
	private static String createTreeObjectScoreLogicOperatorValueExpression(
		...
		if (expressionValue instanceof ExpressionValueTreeObjectReference) {
			...
			droolsValue = getDroolsVariableValueFromExpressionValueTreeObject(
	...
	private static String getVariablesListValuesForFunctions(ExpressionValueCustomVariable leftExpressionCustomVariable,
	...
	private static String negatedExpressions(ExpressionChain prattParserResultExpressionChain)
		...
		if (prattParserResultExpressionChain.getExpressions().get(1) instanceof ExpressionChain) {
			String auxRule = processResultConditionsFromPrattParser(
					(ExpressionChain) prattParserResultExpressionChain.getExpressions().get(1));
		...
		if (prattParserResult != null) {
			...
			if ((prattParserResultExpressionChain.getExpressions().get(0) instanceof ExpressionChain)
				...
				// works fine
					...
					switch (((ExpressionFunction) prattParserResultExpressionChain.getExpressions().get(1))
							.getValue()) {
					case MAX:
					case MIN:
					case AVG:
	...
	private static String parseLogicOperators(List<Expression> expressions, AvailableOperator availableOperator)
			throws NullTreeObjectException, TreeObjectInstanceNotRecognizedException, TreeObjectParentNotValidException,
			NullCustomVariableException, NullExpressionValueException, DateComparisonNotPossibleException {
		StringBuilder droolsConditions = new StringBuilder();
							...
							case DAYS:
								...
								if (value instanceof Timestamp) {
									throw new DateComparisonNotPossibleException(
											unitName + " and Date types incompatible",
	 ...
	 */
	private static String parsePluginMethods(ExpressionChain actions)
			throws NullTreeObjectException, TreeObjectInstanceNotRecognizedException, TreeObjectParentNotValidException,
		...
		for (ExpressionValue<?> parameter : functionParameters) {
			if (parameter instanceof ExpressionValueCustomVariable) {
				...
				stringFunctionParameters
						.add("'" + ((ExpressionValueTreeObjectReference) parameter).getReference().getName() + "'");
