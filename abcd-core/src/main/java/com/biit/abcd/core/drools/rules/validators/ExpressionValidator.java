...
public class ExpressionValidator {
    ...
    private static void checkMethodParameters(ValueType leftVariableFormat, ExpressionChain prattExpressionChain) throws InvalidExpressionException {
        ...
        switch (((ExpressionFunction) prattExpressionChain.getExpressions().get(1)).getValue()) {
            ...
            case MIN:
            case SUM:
                if (!leftVariableFormat.equals(ValueType.NUMBER) && !Objects.equals(parameterType, ValueType.NUMBER)) {
            ...
            case CONCAT_SEPARATOR:
                if (!leftVariableFormat.equals(ValueType.TEXT) && !Objects.equals(parameterType, ValueType.TEXT)) {
                    throw new InvalidExpressionException("Invalid expression '" + prattExpressionChain + "':" + leftVariableFormat + " <-> " + parameterType);
                }
                break;
            case IF:
