...
public class DiagramParser {
	...
	private List<Rule> parseDiagramElement(DiagramElement node, ExpressionChain extraConditions, List<Rule> newRules) throws ExpressionInvalidException,
		...
		case TABLE:
			...
			if (tableNode.getTable() != null) {
				List<DroolsRule> rulesList = TableRuleToDroolsRule.parse(node, tableNode.getTable(), extraConditions);
				for (DroolsRule droolsRule : rulesList) {
					newRules.addAll(RuleToDroolsRule.parse(node, droolsRule, getDroolsHelper()));
				}
			}
			break;
		case RULE:
			...
			if (ruleNode.getRule() != null) {
				...
				newRules.addAll(RuleToDroolsRule.parse(node, ruleNode.getRule(), extraConditions, getDroolsHelper()));
			}
			break;
		case CALCULATION:
		case SINK:
			...
			if (expressionNode.getExpression() != null) {
				...
				newRules.addAll(RuleToDroolsRule.parse(node, rule, null, getDroolsHelper()));
			}
			break;
		case DIAGRAM_CHILD:
	 ...
	 * @throws InvalidRuleException
	 */
	private List<ExpressionChain> completeForkExpressions(DiagramFork forkNode, ExpressionChain previousConditions) throws RuleNotImplementedException,
			InvalidRuleException {
		List<ExpressionChain> forkConditions = new ArrayList<>();
