	...
	private List<Rule> parseDiagramElement(DiagramElement node, ExpressionChain extraConditions, List<Rule> newRules) throws ExpressionInvalidException,
		...
		case RULE:
			...
			if (ruleNode.getRule() != null) {
				...
				RuleChecker.checkRule(ruleNode.getRule());
		...
		case SINK:
			...
			if (expressionNode.getExpression() != null) {
				// Check the rule generated by the user
				RuleChecker.checkRule(new Rule(expressionNode.getExpression().getName(), null, expressionNode.getExpression()));
		...
		case FORK:
			...
			for (ExpressionChain expressionChain : forkConditions) {
				RuleChecker.checkRule(new Rule(expressionChain.getName(), expressionChain, null));
			}
			break;
		default:
			break;
