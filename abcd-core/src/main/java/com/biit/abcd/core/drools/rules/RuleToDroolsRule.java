...
public class RuleToDroolsRule {
    ...
    public static List<DroolsRule> parse(DiagramElement node, Rule rule, ExpressionChain extraConditions, Integer salience, DroolsHelper droolsHelper)
        ...
        if (rule != null) {
            ...
            if (hasGenericVariables(ruleCopy.getConditions())) {
                droolsRuleList = unwrapGenericVariables(ruleCopy, salience);
            }
        ...
        for (DroolsRule droolsRule : conditionsRules) {
            try {
                List<DroolsRule> actionsRules = ExpressionToDroolsRule.parse(node, droolsRule, salience, droolsHelper);
    ...
    private static List<TreeObject> getUnwrappedTreeObjects(ExpressionValueGenericVariable expressionValueGenericVariable) {
        ...
        switch (expressionValueGenericVariable.getType()) {
            case CATEGORY:
                treeObjects = new ArrayList<>(getDroolsHelper().getForm().getAll(Category.class));
                break;
            case GROUP:
                treeObjects = new ArrayList<>(getDroolsHelper().getForm().getAll(Group.class));
                break;
            case QUESTION_CATEGORY:
                List<TreeObject> categories = new ArrayList<>(getDroolsHelper().getForm().getAll(Category.class));
                ...
                break;
            case QUESTION_GROUP:
                List<TreeObject> groups = new ArrayList<>(getDroolsHelper().getForm().getAll(Group.class));
                treeObjects = new ArrayList<>();
    ...
    private static DroolsRule createEndCombinationRule(ITreeElement result, List<DroolsRule> newConditions, DroolsRule droolsRule)
        ...
        DroolsRuleGroupEndRule droolsEngGroupRule;
