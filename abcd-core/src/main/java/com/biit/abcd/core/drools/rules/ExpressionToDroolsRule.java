...
public class ExpressionToDroolsRule {
    ...
    public static List<DroolsRule> parse(DiagramElement node, DroolsRule droolsRule, Integer salience, DroolsHelper droolsHelper) throws ExpressionInvalidException,
        ...
        if (droolsRule.getActions() != null && !droolsRule.getActions().getExpressions().isEmpty()) {
            ...
            if (droolsRule.getActions().getExpressions().get(0) instanceof ExpressionValueGenericCustomVariable) {
                droolsRules = createExpressionDroolsRuleSet(droolsRule, salience);
            } else {
                droolsRules = Collections.singletonList(createExpressionDroolsRule(droolsRule, salience));
            }
        } else {
            droolsRules = Collections.singletonList(droolsRule);
        }
        ...
        if (droolsRules != null && !droolsRules.isEmpty() && hasIfCondition(droolsRules.get(0))) {
            ...
            for (DroolsRule auxRule : auxDroolsRules) {
                droolsRules.addAll(RuleToDroolsRule.parse(node, auxRule, salience, droolsHelper));
            }
        }
    ...
    private static List<DroolsRule> createExpressionDroolsRuleSet(DroolsRule droolsRule, Integer salience) {
        ...
        switch (expressionValueGenericCustomVariable.getType()) {
            ...
            case QUESTION_GROUP:
                for (TreeObject category : expressionValueGenericCustomVariable.getVariable().getForm().getChildren()) {
                    ...
                    // for nested groups
                    Collections.reverse(groups);
                    List<TreeObject> groupQuestions = new ArrayList<>();
        ...
        if (treeObjects != null && !treeObjects.isEmpty()) {
            for (TreeObject treeObject : treeObjects) {
                ...
                // Add to the rule set
                DroolsRule createdDroolsRule = createExpressionDroolsRule(droolsRule, expressionChainCopy, salience);
    ...
    private static List<DroolsRule> parseIfRules(List<DroolsRule> droolsRules) {
        List<DroolsRule> ifRules = new ArrayList<>();
    ...
    private static List<DroolsRule> parseIfRule(DroolsRule droolsRule) {
        ...
        ifThenRule.setSalience(droolsRule.getSalience());
        ...
        ifElseRule.setSalience(droolsRule.getSalience());
    ...
    private static DroolsRule generateDroolsRule(DroolsRule droolsRule) {
        ...
        newDroolsRule.setSalience(droolsRule.getSalience());
        return newDroolsRule;
    }
