...
public class DroolsFunctionsTest {
	...
	@Test(groups = { "externalDroolsFunctions" })
	public void rulesTest() {
		try {
			String drlFile = readFile("./src/test/resources/droolsFunctionsTest.drl", StandardCharsets.UTF_8);
			// Execution of the rules
			DroolsForm droolsForm = runDroolsRules(drlFile);
			if (submittedForm != null) {
				// Check result
				System.out.println("Variable value: "
						+ droolsForm.getSubmittedForm().getVariableValue(CUSTOM_VARIABLE_RESULT));
			}
		} catch (Exception e) {
			AbcdLogger.errorMessage(this.getClass().getName(), e);
		}
