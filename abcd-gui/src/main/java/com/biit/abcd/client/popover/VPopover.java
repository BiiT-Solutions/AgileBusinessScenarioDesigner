...
public class VPopover extends com.vaadin.client.ui.VWindow {
	...
	private static final int MIN_EDGE_DISTANCE = 0;
	...
	public void slideIn() {
		...
		if (relComponent != null) {
			boolean isCloseToBottom = relComponent.getAbsoluteTop() - Window.getScrollTop() > Window.getClientHeight() / 2;
			if (isCloseToBottom) {
	...
	public void showNextTo(Widget paintable) {
		...
		final int centerOfReferencComponent = paintable.getAbsoluteLeft() + paintable.getOffsetWidth() / 2
		...
		if (getOffsetWidth() < Window.getClientWidth()) {
			...
			} else if (left + getOffsetWidth() + MIN_EDGE_DISTANCE > Window.getClientWidth()) {
				left = Window.getClientWidth() - getOffsetWidth() - MIN_EDGE_DISTANCE;
			}
	...
	private void showReferenceArrow(boolean onTop, int centerOfReferencComponent, Widget ref) {
		UIObject.setStyleName(arrowElement, "v-touchkit-popover-pointer-bottom", !onTop);
		UIObject.setStyleName(arrowElement, "v-touchkit-popover-pointer", onTop);

		int horizontalpoint = centerOfReferencComponent - arrowElement.getOffsetWidth() / 2;
		if (horizontalpoint - MIN_ARROW_EDGE_DISTANCE < 0) {
			horizontalpoint = MIN_ARROW_EDGE_DISTANCE;
		} else if (horizontalpoint + arrowElement.getOffsetWidth() + MIN_ARROW_EDGE_DISTANCE > Window.getClientWidth()) {
			horizontalpoint = Window.getClientWidth() - arrowElement.getOffsetWidth() - MIN_ARROW_EDGE_DISTANCE;
		}
	...
	private boolean fitsBelow(Widget paintable) {
		final int spaceBelow = Window.getClientHeight()
				- (paintable.getAbsoluteTop() + paintable.getOffsetHeight() - Window.getScrollTop());
		final int requiredHeight = getOffsetHeight() + arrowElement.getOffsetHeight();
		return spaceBelow > requiredHeight;
	}

	private boolean fitsAbove(Widget paintable) {
		final int spaceOntop = paintable.getAbsoluteTop() - Window.getScrollTop();
		final int requiredHeight = getOffsetHeight() + arrowElement.getOffsetHeight();
