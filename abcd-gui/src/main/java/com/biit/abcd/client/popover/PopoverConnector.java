...
public class PopoverConnector extends WindowConnector implements NativePreviewHandler {
	...
	public void onStateChanged(StateChangeEvent stateChangeEvent) {
		...
		ComponentConnector relatedConnector = (ComponentConnector) getState().getRelatedComponent();
		getWidget().setRelatedComponent(relatedConnector == null ? null : relatedConnector.getWidget());
	...
	protected void updateComponentSize() {
		...
		if (getState().isFullscreen()) {
			...
			Scheduler.get().scheduleDeferred(new ScheduledCommand() {
				public void execute() {
					getWidget().getModalityCurtain().addClassName("v-touchkit-opacity-transition");
	...
	public void onPreviewNativeEvent(NativePreviewEvent event) {
		...
		if (getWidget().getModalityCurtain().isOrHasChild(targetElement) && getWidget().isClosable()
	...
	protected void init() {
		...
		getLayoutManager().addElementResizeListener(getConnection().getUIConnector().getWidget().getElement(),
				resizeListener);
	}
	...
	public void onUnregister() {
		...
		getLayoutManager().removeElementResizeListener(getConnection().getUIConnector().getWidget().getElement(),
				resizeListener);
	}
