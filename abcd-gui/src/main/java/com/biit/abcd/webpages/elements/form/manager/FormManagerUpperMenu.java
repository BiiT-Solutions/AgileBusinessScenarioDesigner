...
public class FormManagerUpperMenu extends UpperMenu {
    ...
    private IconButton newButton, newFormButton, newVersion, copyForm, export, exportToDrools, testScenarios, createTestScenario,
    ...
    private void defineMenu() {
        newButton = addSubMenu(ThemeIcon.FORM_MANAGER_NEW, LanguageCodes.FORM_MANAGER_FORM,
                LanguageCodes.FORM_MANAGER_FORM, createNewFormButtons());
        newButton.setHeight("100%");
        newButton.setWidth(BUTTON_WIDTH);
        addIconButton(newButton);

        List<IconButton> exportButtons = new ArrayList<>();
        ...
        export = addSubMenu(ThemeIcon.FORM_MANAGER_EXPORT, LanguageCodes.FORM_MANAGER_EXPORT,
                LanguageCodes.FORM_MANAGER_EXPORT, exportButtons);
        export.setHeight("100%");
        export.setWidth(BUTTON_WIDTH);
        addIconButton(export);

        List<IconButton> testScenarioButtons = new ArrayList<>();
        ...
        testScenarioButtons.add(createTestScenario);
        testScenarioButtons.add(launchTestScenario);

        testScenarios = addSubMenu(ThemeIcon.TEST_SCENARIOS, LanguageCodes.FORM_MANAGER_TEST_SCENARIOS,
                LanguageCodes.FORM_MANAGER_TEST_SCENARIOS, testScenarioButtons);
        testScenarios.setHeight("100%");
        testScenarios.setWidth(BUTTON_WIDTH);
        addIconButton(testScenarios);
    ...
    private List<IconButton> createNewFormButtons() {
        ...
        copyForm.setWidth(BUTTON_WIDTH);
        iconButtonList.add(copyForm);
    ...
    public void updateCopyFormButton(SimpleFormView selected) {
        if (selected != null && !(selected instanceof RootForm)) {
            copyForm.setEnabled(getSecurityService().isAuthorizedToForm(selected.getOrganizationId(),
                    UserSessionHandler.getUser())
                    && !getSecurityService().isFormAlreadyInUse(selected.getId(), UserSessionHandler.getUser()));
        } else {
            copyForm.setEnabled(false);
        }
    }
    ...
    public void updateExportJsonFormButton(SimpleFormView selected) {
        // Only some users can remove forms.
        try {
            exportJson.setVisible(getSecurityService().isUserAuthorizedInAnyOrganization(UserSessionHandler.getUser(),
                    AbcdActivity.FORM_EXPORT));
        } catch (UserManagementException e) {
            exportJson.setVisible(false);
        }
        // When visible, enabled when can delete an element.
        exportJson.setEnabled(
                selected != null && !(selected instanceof RootForm) && getSecurityService().isAuthorizedActivity(
